...
const serverUrl = "https://rnyrgntghdj9.usemoralis.com:2053/server";
const appId = "1toxBCzhGbaFAjJj82oVwKc02O5ZhFhE7hrev1je";
...
//web3 constants
const contractAddress = "0x471355C92De3Ee8c7Da3230f7440A5aD2ebbE8c9";
const contractABI = [
	{
		inputs: [],
...
function drawCanvas() {
	...
	worldImage.src = "static/img/Myland.png";
	worldImage.onload = () => {
		initializeMap();
	};
}
...
async function assignPlot() {
	...
	if (!assigned) {
		const metadata = {
			...
			image: "https://thumbs.dreamstime.com/b/cartoon-land-20382798.jpg",
		};
...
async function mint(_tokenURI) {
	...
	try {
		...
		displayMessage(
			"00",
			"Transaction confirmed with hash " + transaction.hash
		);
	} catch (error) {
